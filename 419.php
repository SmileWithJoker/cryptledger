<?php
require "includes/header.php";
?>

<body>
    <div class="d-flex align-items-center justify-content-center min-vh-100 bg-gray-100">
        <div class="text-center p-4">
            <div class="mb-4">
                <!-- Custom SVG Illustration for a 419 error -->
                <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto h-48 w-48 text-primary" viewBox="0 0 24 24"
                    fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12A10 10 0 0 1 12 2z" />
                    <path d="M12 6v6l4 2" />
                </svg>
            </div>
            <h1 class="display-3 fw-bold text-dark">419</h1>
            <p class="h5 fw-normal text-muted mb-4">Page Expired</p>
            <p class="text-muted mb-5">
                The page has expired due to inactivity. Please refresh and try again.
            </p>
            <a class="btn custom-btn-2" href="index.php" role="button">
                Go to Homepage
            </a>
        </div>
    </div>
</body>

<?php
require "includes/footer.php";
?>
